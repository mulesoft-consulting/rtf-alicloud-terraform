...
# Constants
SCRIPT_VERSION='20200728-136beef'
...
function configure_ip_tables() {
  ...
  if [[ $VERSION_ID == 8* ]]; then
    echo "Detected OS version: $VERSION_ID, Skip configuring iptables rules."
...
function verify_outbound_connectivity() {
    ...
    HTTP_PROXY="$RTF_HTTP_PROXY" ${RTFCTL} test outbound-network --use-env-proxy --mutual-tls-check=false
}
